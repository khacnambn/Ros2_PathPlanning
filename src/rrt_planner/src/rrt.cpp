 ...
 */
RRT::RRT(nav2_costmap_2d::Costmap2D* costmap, int sample_num, double max_dist)
  : PlannerCore(costmap), sample_num_(sample_num), max_dist_(max_dist)
{
}
...
bool RRT::plan(const Node& start, const Node& goal, std::vector<Node>& path, std::vector<Node>& expand)
{
  ...
  while (iteration < sample_num_)
  {
    ...
    // obstacle
    if (costmap_->getCharMap()[sample_node.id()] >= 254 * factor_) //nav2_costmap_2d::LETHAL_OBSTACLE = 254
      continue;
...
bool RRT::_isAnyObstacleInPath(const Node& n1, const Node& n2)
{
  ...
  for (int i = 0; i < n_step; i++)
  {
    ...
    if (costmap_->getCharMap()[grid2Index(static_cast<int>(line_x), static_cast<int>(line_y))] >=
        254 * factor_) //nav2_costmap_2d::LETHAL_OBSTACLE = 254
      return true;
  }
  return false;
}
